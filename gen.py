...
# Vi du: https://www.youtube.com/playlist?list=PLxxxxxx
# Chi copy phan sau "list="
PLAYLIST_ID = "PUT_YOUR_PLAYLIST_ID_HERE"
...
# ============================================
# 3 NHOM NOI DUNG ( dinh nghi sacc truo fc )
# Nhon: Danh sach cac video thuoc nhom
# ============================================

# Danh sach nhom - chinh sua theo nhu cau thuc te
GROUPS = {
    "00_Co_Ban": {
        "name": "Co Ban - Bat dau tu con so 0",
        "description": "Cac bai co ban ve moi gioi bat dong san",
        "video_ids": []  # De trong neu tu dong loc theo playlist
    },
    "01_Chuyen_Sau": {
...
# ============================================
# DEMO DATA (dung khi khong co API Key)
# ============================================
DEMO_VIDEOS = [
    {
        "id": "demo_001",
...
def generate_video_markdown(video, vault_path, all_files):
    ...
    # Them wiki links den cac video khac
    for f in all_files:
        if f != filename:
...
def assign_to_group(video_idx, total, groups_config):
    """Phan nhom video theo chi so (chi can display, k dung video_ids)"""
...
def build_vault(playlist_id, api_key, vault_name, groups):
    ...
    # Buoc 1: Tao file cho tung video
    all_files = []
    ...
    total_files = len(all_files) + len(group_toc_files) + 2  # +2: index + metadata
    print(f"\n===== HOAN THANH =====")
...
def create_index_file(group_toc_files, group_videos, vault_path):
    ...
    for group_key in groups.keys():
        toc_file = [f for f in group_toc_files if group_key.replace("_", " ").title().replace(" ", "_") in f
                    or group_key.split("_")[1] in f or group_key.split("_")[2] in f][0]
